...
def get_latest_checkpoint(log_dir):
    ...
    zip_files = glob.glob(os.path.join(log_dir, "hover_v2_sac_*.zip"))
    if not zip_files:
        return None
    
    valid_checkpoints = []
    for zip_path in zip_files:
        match = re.search(r'hover_v2_sac_(\d+)_steps\.zip$', os.path.basename(zip_path))
        if not match:
            continue
            
        steps = int(match.group(1))
        pkl_path = os.path.join(log_dir, f"hover_v2_sac_vecnormalize_{steps}_steps.pkl")
