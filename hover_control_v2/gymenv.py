...
class GymEnv(gymnasium.Env):
    '''Gymnasium environment class for drone hover using attitude control (Flight Mode 3).
    
    Flight Mode 3: [p, q, r, z]
    - p, q: Roll and pitch angles
    - r: Yaw angle
    - z: Altitude
    '''
    def __init__(
        ...
        self.max_roll_pitch_angle = 30.0 * np.pi / 180.0
        self.max_yaw_angle = 180.0 * np.pi / 180.0
        self.min_z = 0.0
        self.max_z = 5.0
    ...
    def step(self, action: np.ndarray) -> tuple[Any, float, bool, bool, dict[str, Any]]:
        ...
        # Action: [roll, pitch, yaw, z]
        scaled_action = action.copy()
        scaled_action[0] *= self.max_roll_pitch_angle
        scaled_action[1] *= self.max_roll_pitch_angle
        scaled_action[2] *= self.max_yaw_angle
        # Altitude: Scale [-1, 1] to [min_z, max_z]
        scaled_action[3] = (scaled_action[3] + 1.0) / 2.0 * (self.max_z - self.min_z) + self.min_z
